...
- [ ] **B1.4** Create `apps/api/src/routes/biz.ts` with `app.addHook('preHandler', authMiddleware)`, the `requireBusinessOwner(userId)` helper, and `GET /biz/me`. Register in `apps/api/src/app.ts`. Add `apps/api/tests/routes/biz-onboarding.test.ts` covering 200 (owner) + 200 with `business:null` (no owned business).
