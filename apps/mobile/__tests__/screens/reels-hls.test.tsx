...
describe('Reels feed prefers the HLS manifest when present', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('passes the master.m3u8 to useVideoPlayer (not the 720p variant)', async () => {
    render(<ReelsScreen />);
    await waitFor(() => {
      expect(useVideoPlayer).toHaveBeenCalledWith({ uri: 'https://cdn/master.m3u8' }, expect.any(Function));
    });
    expect(useVideoPlayer).not.toHaveBeenCalledWith({ uri: 'https://cdn/720.m3u8' }, expect.any(Function));
